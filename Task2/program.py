...
def plot_model_and_datasets(net, eval_data):
    ...
    #plt.savefig("Model_and_datasets.png")
    #plt.show()
    time.sleep(0.2)
...
def main():
    ...
    #use the dataset argumentation function to generate training data
    data_number = 1600
    batch_number = 16
    repeat_number = 1
    ...
    #visualize the initialized network function and training dataset
    x_model_label = np.array([-10, 10, 0.1])
    y_model_label = (x_model_label * Tensor(model_params[0]).asnumpy()[0][0] + Tensor(model_params[1]).asnumpy()[0])
    plt.axis([-10, 10, -20, 25])
    plt.scatter(x_eval_label, y_eval_label, color="red", s=5)
    plt.plot(x_model_label, y_model_label, color="blue")
    plt.plot(x_target_label, y_target_label, color="green")
    plt.savefig("Model_and_objective_funcs.png")
